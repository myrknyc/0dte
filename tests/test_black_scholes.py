...
def run_all():
    ...
    try:
        ...
        print("OK imported black_scholes function")
    ...
    try:
        ...
        if 4.5 <= call_price <= 6.5:
            print(f"    OK Price is reasonable (expected ~$5-6)")
            tests_passed += 1
        else:
            print(f"    FAIL Price outside expected range [4.5, 6.5]")
            tests_failed += 1
    ...
    try:
        ...
        if 3.0 <= put_price <= 5.0:
            print(f"    OK Price is reasonable")
            tests_passed += 1
        else:
            print(f"    FAIL Price outside expected range [3.0, 5.0]")
            tests_failed += 1
    ...
    try:
        ...
        if abs(lhs - rhs) < 1e-6:
            print(f"    OK Put-call parity holds")
            tests_passed += 1
        else:
            print(f"    FAIL Put-call parity violated")
            tests_failed += 1
    ...
    try:
        ...
        if call_itm >= intrinsic and call_itm <= intrinsic + 3:
            print(f"    OK ITM call >= intrinsic value")
            tests_passed += 1
        else:
            print(f"    FAIL ITM call price not reasonable")
            tests_failed += 1
    ...
    try:
        ...
        if 0 < call_otm < 2.0:
            print(f"    OK OTM call has low value")
            tests_passed += 1
        else:
            print(f"    FAIL OTM call price not reasonable")
            tests_failed += 1
    ...
    try:
        ...
        if is_decreasing:
            print(f"    OK Call prices decrease with strike (monotonic)")
            tests_passed += 1
        else:
            print(f"    FAIL Call prices not monotonic")
            tests_failed += 1
    ...
    try:
        ...
        if abs(call_expired - 5.0) < 0.01 and abs(put_expired - 5.0) < 0.01:
            print(f"    OK Expired options = intrinsic value")
            tests_passed += 1
        else:
            print(f"    FAIL Expired option values incorrect")
            tests_failed += 1
